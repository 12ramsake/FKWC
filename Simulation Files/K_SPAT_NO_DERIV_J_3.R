...
simulate_mv_depths<-function(N1,N2,N3,c1,c2,c3,res=100,
  ...
  KFSD_depth=K_spatial_depth(fdat,N1+N2+N3)
  return(  KFSD_depth)
...
for(N1 in c(50,100)){
  ...
  for(dist in c("N","t","sn")){
    for(betai in 2:15){
      beta=tmp[betai]
      fileName=paste0("FKWCResults/kspat_dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_p05_p05_beta_p5_p",betai,"_p5_N_",N1,"_num_runs_200.Rda",sep="")
  ...
  for(dist in c("N","t","sn")){
    for(alphai in 2:15){
      alpha=tmp[alphai]
      fileName=paste0("FKWCResults/kspat_dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_",alphai,"_p05_beta_p5_p5_p5_N_",N1,"_num_runs_200.Rda",sep="")
