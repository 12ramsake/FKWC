...
make_Plots=function(xs=bet,leg=T){
  ...
  legend(xs[12,1],0.7,legend=c(expression(italic(W)),expression(italic(M))),col=cols,lty=1,cex=1.5)
  # legend(xs[14,1],0.7,legend=c(expression(italic(W)),expression(italic(M))),col=cols,lty=1,cex=1.5)
}
...
make_Plots();  sizes=prop_rejs[1,1:10]
...
for(dist in c("N","t","sn")){
  for(ratio in ratios){
    ...
    load(FN)
    size=rbind(size, prop_rejs[1,1:10])
    t_pr_2=rbind(t_pr_2,prop_rejs[2,1:10])
  }
}
# }
t_pr_2
