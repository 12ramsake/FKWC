...
make_Plots=function(xs=bet,leg=T){
  ...
  matplot(xs[,1:5],prop_rejs[,1:5],type='l',
          col=rainbow(5),
          lty=1,
          lw=5,bty="n",
          cex.axis=2,main="",
          ...
          xlab="")
  if(leg)
    legend("bottomright",legend=colnames(prop_rejs)[c(1:5)],lty=1,col=rainbow(5),lwd=3,cex=2)
}
...
make_Plots();  sizes=prop_rejs[1,c(6:10)]
...
for(N1 in c(50,100)){
  ...
  t_pr_2=cbind(t_pr_2,t_pr[c(6:10),])
...
for(dist in c("N","t","sn")){
  for(ratio in ratios){
    ...
    load(FN)
    size=rbind(size, prop_rejs[1,c(6:10)])
    t_pr_2=rbind(t_pr_2,prop_rejs[2,c(6:10)])
  }
}
# }
t_pr_2
...
for(out in 1:4){
  ...
  # print(t(prop_rejs)[c(1:5),]-t(prop_rejs)[c(6:10),])
  print(t(prop_rejs)[c(6:10),])
