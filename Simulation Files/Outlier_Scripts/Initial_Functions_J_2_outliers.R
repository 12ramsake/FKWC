...
generate_outliers=function(dat,N1,N2,contamination_level=0.1,outlier_type=1,both_samples=T){
  ...
  # con1 =1:contamination_level*(N1+N2)
  if(both_samples)
  ...
  if(outlier_type==1)
    dat$mdata[c(con1,con2),]=t(orig)+2*grid
...
runMVSim<-function(both,out,contamination_level,N1,N2,c1,c2,grid,num_runs,fileName,dist="t",del=0.9){
    ...
    depth_values=try({foreach(i=1:num_runs) %dopar% {simulate_mv_depths(both,out,contamination_level,N1,N2,c1,c2,res=100,del=del,dist=dist,grid=grid)}})
