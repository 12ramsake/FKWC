#Source the initial script J2

#simulate data and the resulting depths using the derivative
simulate_mv_depths<-function(both,out,contamination_level,N1,N2,c1,c2,res=100,
                             grid=seq( 0, 1, length.out = res ),
                             dist="t",
                             del=0.9){
  
  dat=gfd(N1,N2,Cov1=c1,Cov2=c2,res=res,dist,del)
  dat=generate_outliers(dat,N1,N2,contamination_level,outlier_type=out,both_samples=both)
  fdat<-fdata(dat$mdata,dat$argvals)
  # matplot(dat$argvals,t(dat$mdata)[,(1):(N1)],type='l',col=1)
  # N1=N2=10
  # matplot(dat$argvals,t(dat$mdata),type='l',col=c(1,rep(3,9)),lwd=2)
  # matlines(dat$argvals,t(dat$mdata)[,(N1+1):(N1+N2)],col=2)
  
  # depth.RPD() implements a depth measure based on random projections possibly using several derivatives (see Cuevas et al. 2007).
  RPD_depth=depth.RP(fdat,dfunc = "Liu1")$dep
  #fraiman and muniz, really slow
  FM_depth=depth.FM(fdat)$dep
  #band depth
  MBD_depth=MBD(dat$mdata)
  LTR_depth=c(norm.fdata(fdat))
  
  depths=data.frame(FM_depth,RPD_depth,MBD_depth,LTR_depth)
  
  return(depths)
  
}


contamination_level=0.1
cl=contamination_level*100

# N1 =100
# both=T
# out=4
# N2=N1
# # 



for(N1 in c(50,100,250)){
  for(both in c(T,F)){
    for(out in 1:4){
      N2=N1
      print("sim 1")
      #changes in beta
      count=0
      k1=function(t1,t2){k_exp(t1,t2,.5,.05)}
      c1=getCov(grid,k1)
      tmp=seq(0.5,1,length.out = 15)
      dist="N"
      
      for(betai in 1:15){
        # for(betai in 8){
          beta=tmp[betai]
          fileName=paste0("FKWCResults/cl_",cl,"_both_",both,"_out_",out,"_dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_p05_beta_p",betai,"_p5_N_",N1,"_num_runs_200.Rda",sep="")
          k2=function(t1,t2){k_exp(t1,t2,beta,.05)}
          c2=getCov(grid,k2)
          runMVSim(both,out,contamination_level,N1,N2,c1,c2,grid,num_runs,fileName,dist=dist)
          count=count+1
          print(count/(14*3))
        }
      
      
      
      
      print("sim 2")
      #changes in alpha
      count=0
      k1=function(t1,t2){k_exp(t1,t2,0.5,.05)}
      c1=getCov(grid,k1)
      tmp=seq(0.05,0.1,length.out=15)
     
      for(alphai in 1:15){
          alpha=tmp[alphai]
          fileName=paste0("FKWCResults/cl_",cl,"both_",both,"_out_",out,"_dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_",alphai,"_beta_p5_p5_N_",N1,"_num_runs_200.Rda",sep="")
          k2=function(t1,t2){k_exp(t1,t2,0.5,alpha)}
          c2=getCov(grid,k2)
          runMVSim(both,out,contamination_level,N1,N2,c1,c2,grid,num_runs,fileName,dist=dist)
          count=count+1
          print(count/(14*3))
        
      }
    }   
  }
}













contamination_level=0.1
cl=contamination_level*100

for(N1 in c(50,100,250)){
  for(both in c(F)){
    for(out in 1:4){
      N2=N1
      print("sim 1")
      #changes in beta
      count=0
      k1=function(t1,t2){k_exp(t1,t2,.5,.05)}
      c1=getCov(grid,k1)
      tmp=seq(0.5,1,length.out = 15)
      dist="N"
      
      for(betai in 1){
        beta=tmp[betai]
        fileName=paste0("FKWCResults/cl_",cl,"_both_",both,"_out_",out,"_dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_p05_beta_p",betai,"_p5_N_",N1,"_num_runs_200.Rda",sep="")
        k2=function(t1,t2){k_exp(t1,t2,beta,.05)}
        c2=getCov(grid,k2)
        runMVSim(both,out,contamination_level,N1,N2,c1,c2,grid,num_runs,fileName,dist=dist)
        count=count+1
        print(count/(14*3))
      }
      
      
      
      
      print("sim 2")
      #changes in alpha
      count=0
      k1=function(t1,t2){k_exp(t1,t2,0.5,.05)}
      c1=getCov(grid,k1)
      tmp=seq(0.05,0.1,length.out=15)
      
      for(alphai in 1){
        alpha=tmp[alphai]
        fileName=paste0("FKWCResults/cl_",cl,"both_",both,"_out_",out,"_dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_",alphai,"_beta_p5_p5_N_",N1,"_num_runs_200.Rda",sep="")
        k2=function(t1,t2){k_exp(t1,t2,0.5,alpha)}
        c2=getCov(grid,k2)
        runMVSim(both,out,contamination_level,N1,N2,c1,c2,grid,num_runs,fileName,dist=dist)
        count=count+1
        print(count/(14*3))
        
      }
    }   
  }
}












# 
# 
# 
# 
# 
# 
# 
# 
# for(N1 in c(50, 100, 250)) {
#   for (both in c(T, F)) {
#     for (out in 1:4) {
#       N2 = N1
#       print("sim 1")
#       #changes in beta
#       count = 0
#       k1 = function(t1, t2) {
#         k_exp(t1, t2, .5, .05)
#       }
#       c1 = getCov(grid, k1)
#       tmp = seq(0.5, 1, length.out = 15)
#       
#       for (dist in c("N")) {
#         for (betai in 1) {
#           beta = tmp[betai]
#           fileName = paste0( "FKWCResults/cl_",cl,"both_",both,"_out_",out,"_dist_", dist,"_noderiv_depths_k_exp_alpha1_p05_p05_beta_p", betai, "_p5_N_",N1,"_num_runs_200.Rda", sep = "")
#           k2 = function(t1, t2) {
#             k_exp(t1, t2, beta, .05)
#           }
#           c2 = getCov(grid, k2)
#           runMVSim(both,out,contamination_level,N1, N2, c1, c2,grid, num_runs,fileName, dist = dist)
#           count = count + 1
#           print(count / (14 * 3))
#         }
#       }
#       
#       
#       
#       print("sim 2")
#       #changes in alpha
#       count = 0
#       k1 = function(t1, t2) {
#         k_exp(t1, t2, 0.5, .05)
#       }
#       c1 = getCov(grid, k1)
#       tmp = seq(0.05, 0.1, length.out = 15)
#       for (dist in c("N")) {
#         for (alphai in 1) {
#           alpha = tmp[alphai]
#           fileName = paste0(
#             "FKWCResults/cl_",cl,"both_",
#             both,"_out_",out,"_dist_", dist,"_noderiv_depths_k_exp_alpha1_p05_",alphai,"_beta_p5_p5_N_",N1, "_num_runs_200.Rda",
#             sep = ""
#           )
#           k2 = function(t1, t2) {
#             k_exp(t1, t2, 0.5, alpha)
#           }
#           c2 = getCov(grid, k2)
#           runMVSim(both,out,contamination_level,N1, N2, c1, c2,grid, num_runs,fileName, dist = dist)
#           count = count + 1
#           print(count / (14 * 3))
#         }
#       }
#     }
#   }
# }
# 
# 
# 
# 





