...
simulate_mv_depths<-function(N1,N2,c1,c2,M,res=100,grid=seq( 0, 1, length.out = res ),dist="t", del=0.9){
  ...
  RPD_depth_Like=depth.RPD(fdat, nproj = M)$dep
  RPD_depth_SD=depth.RPD(fdat, dfunc2='mdepth.SD',nproj = M)$dep
  
  depths=data.frame(RPD_depth_SD,RPD_depth_Like)
...
runMVSim<-function(N1,N2,c1,c2,M,grid,num_runs,fileName,dist="t",del=0.9){
  ...
  depth_values=try({foreach(i=1:num_runs) %dopar% {simulate_mv_depths(N1,N2,c1,c2,M,res=100,del=del,dist=dist,grid=grid)}})
...
for(M in c(50,75,100)){
  ...
  for(dist in c("N","t","sn")){
    for(betai in 1:15){
      beta=tmp[betai]
      fileName=paste0("FKWCResults/Directions_",M,"_dist_",dist,"_deriv_depths_k_exp_alpha1_p05_p05_beta_p",betai,"_p5_N_",N1,"_num_runs_200.Rda",sep="")
      k2=function(t1,t2){k_exp(t1,t2,beta,.05)}
      c2=getCov(grid,k2)
      runMVSim(N1,N2,c1,c2,M,grid,num_runs,fileName,dist=dist)
      count=count+1
      print(count/(14*3))
    }
  }
  ...
  for(dist in c("N","t","sn")){
    for(alphai in 1:15){
      alpha=tmp[alphai]
      fileName=paste0("FKWCResults/Directions_",M,"_dist_",dist,"_deriv_depths_k_exp_alpha_p05_",alphai,"_beta_p5_p5_N_",N1,"_num_runs_200.Rda",sep="")
