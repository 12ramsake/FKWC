...
for(N1 in c(50,100,250)){
  ...
  # for(dist in c("t")){
    for(betai in 1:6){
      beta=tmp[betai]
      fileName=paste0("FKWCResults/kspat_dist_",dist,"_deriv_depths_k_exp_alpha1_p05_p05_beta_p",
                      betai,"_p5_N_",N1,"_num_runs_",num_runs,".Rda",sep="")
  ...
  # for(dist in c("t")){
    for(alphai in 2:6){
      alpha=tmp[alphai]
      fileName=paste0("FKWCResults/kspat_dist_",dist,"_deriv_depths_k_exp_alpha_p05_",
