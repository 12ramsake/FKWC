...
simulate_mv_depths<-function(N1=10,N2=10,N3=10,
  ...
  depths=data.frame(RPD_depth_SD,LTR_depth)
  return(depths)
...
for(N1 in c(50,100)){
  ...
  for(dist in c("N","t","sn")){
    # for(dist in c("t")){
      for(betai in 2:15){
      beta=tmp[betai]
      fileName=paste0("FKWCResults/dist_",dist,"_deriv_SIM_TD_k_exp_alpha1_p05_p05_p05_beta_p5_p",betai,"_p5_N_",N1,"_num_runs_200.Rda",sep="")
  ...
  for(dist in c("N","t","sn")){
    ...
    for(alphai in 2:15){
      alpha=tmp[alphai]
      fileName=paste0("FKWCResults/dist_",dist,"_deriv_SIM_TD_k_exp_alpha1_p05_",alphai,"_p05_beta_p5_p5_p5_N_",N1,"_num_runs_200.Rda",sep="")
