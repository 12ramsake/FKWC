...
oneRun<-function(N1,N2,c1,c2,res=100,
  ...
  # filled_in=zoo::na.approx(dat$mdata)
...
N1=100
  ...
  for(dist in c("N","t")){
    for(betai in 1:15){
      beta=tmp[betai]
      fileName=paste0("missing_dist_",dist,"_other_k_exp_alpha1_p05_p05_beta_p",betai,"_p5_N_",N1,"_num_runs_200.Rda",sep="")
  ...
  for(dist in c("N","t")){
    for(alphai in 1:15){
      alpha=tmp[alphai]
      fileName=paste0("missing_dist_",dist,"_other_k_exp_alpha1_p05_",alphai,"_beta_p5_p5_N_",N1,"_num_runs_200.Rda",sep="")
