...
#groups is the group indices at which each sample starts and N+1 is at the end
# getKW<-function(depths,groups,k,N){
#   ranks=rank(depths,ties.method = "average")
#   var=12/(N*(N+1))
#   ts=-3*N-3
#   for(g in 1:k){
#     ts=ts+var*(groups[g+1]-groups[g])* mean(ranks[groups[g]:(groups[g+1]-1)])^2
#   }
#    
#   return(ts)
# }
...
# 
# #takes matrix of depths, and then gets ts for each
# getKW_all_depths<-function(depths,groups,k,N){
#   apply(depths,2,getKW,groups,k,N)
# }
...
for(N in c(50,100,250)*2){
  ...
  for(dist in c("N","t","sn")){
    for(betai in c(1,7,15)){
      ...
      test_stats=lapply(depth_values,getKWPM_all_depths,groups=groups,r=1,k=2,N=N)
      ...
      fileName=paste0("REVISION_RESULTS/DGS/dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_p05_beta_p",betai,"_p5_N1_",N1,"_N2_",N2,"_num_runs_200.Rda",sep="")
      load(fileName)
      depth_values1=depth_values
      ...
      fileName=paste0("REVISION_RESULTS/DGS/dist_",dist,"_other_k_exp_alpha1_p05_p05_beta_p",betai,"_p5_N1_",N1,"_N2_",N2,"_num_runs_200.Rda",sep="")      
      load(fileName)
    ...
    bet=matrix(tmp[c(1,7,15)],ncol=ncol(prop_rejs),nrow=nrow(prop_rejs))
    print(prop_rejs)
...
for(N in c(50,100,250)*2){
  ...
  for(dist in c("N","t","sn")){
    for(alphai in c(1,7,15)){
      ...
      fileName=paste0("REVISION_RESULTS/DGS/dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_",alphai,"_beta_p5_p5_N1_",N1,"_N2_",N2,"_num_runs_200.Rda",sep="")
      load(fileName)
      depth_values1=depth_values
      ...
      ##other tests, had to run guo seperately here
      fileName=paste0("REVISION_RESULTS/DGS/dist_",dist,"_other_k_exp_alpha1_p05_",alphai,"_beta_p5_p5_N1_",N1,"_N2_",N2,"_num_runs_200.Rda",sep="")      
      load(fileName)
      pvalues=do.call(rbind, p_values)
...
for(N1 in c(50,100)){
  ...
  for(dist in c("N","t","sn")){
    for(betai in c(1,7,15)){
      ...
      fileName=paste0("REVISION_RESULTS/DGS/dist_",dist,"_noderiv_depths_k_exp_alpha1_p05_p05_p05_beta_p5_p",betai,"_p5_N1_",N1,"_N2_",N2,"_N3_",N3,"_num_runs_200.Rda",sep="")
      load(fileName)
      depth_values1=depth_values
      ...
      fileName=paste0("REVISION_RESULTS/DGS/dist_",dist,"_other_k_exp_alpha1_p05_p05_p05_beta_p5_p",betai,"_p5_N1_",N1,"_N2_",N2,"_N3_",N3,"_num_runs_200.Rda",sep="")
      load(fileName)
      pvalues=do.call(rbind, p_values)
